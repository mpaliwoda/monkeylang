...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("function has wrong parameteres. Parameters=%+v", fn.Parameters)
	}
...
func TestStringLiteral(t *testing.T) {
	...
	if str.Value != "Hello world" {
		t.Errorf("String has wrong vlaue. got=%q", str.Value)
	}
}
...
func TestBuiltinFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		...
		case []int64:
			...
			for ix, item := range expected {
				testIntegerObject(t, arrObj.Elements[ix], item)
			}
		case nil:
			testNullObject(t, evaluated)
		}
...
func TestHashLiterals(t *testing.T) {
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf(
			"Hash has wrong num of pairs. expected=%d, got=%d",
			len(expected),
			len(result.Pairs),
		)
	}
